...
# @frappe.whitelist(allow_guest=True)
# def gupshup_webhook():
#     """Handle incoming webhooks from Gupshup WhatsApp API"""
#     if frappe.request and frappe.request.data:
#         try:
#             # Parse the webhook data
#             webhook_data = json.loads(frappe.request.data)
...
#                 elif webhook_data["type"] == "message-event":
#                     process_message_status(webhook_data)
            
#             return {"success": True}
#         except Exception as e:
#             frappe.log_error(f"WhatsApp Webhook Error: {str(e)}", "WhatsApp Webhook Failed")
#             return {"success": False, "error": str(e)}
...
#         # Find or create contact
#         contact_name = find_or_create_contact(phone_number)
...
#             "doctype": "WhatsApp Message",
#             "contact": contact_name,
#             "source": phone_number,           # Required field: sender's number
#             "destination": business_number,   # Required field: your business number
#             "message": content,               # Required field: message content
#             "direction": "Incoming",
#             "message_type": message_type,
#             "message_id": message_id,
#             "status": "Delivered",
#             "timestamp": timestamp,
#             "content": content                # Keep this if you have both 'message' and 'content' fields
#         })
...
#         # Update last message date on contact (Fixed DocType name)
#         frappe.db.set_value("Whatsapp Contact", contact_name, "last_message_date", timestamp)
        
#         # Check if this is a response to a campaign
#         #check_campaign_response(contact_name, content)
        
#         return True
#     except Exception as e:
#         frappe.log_error(f"WhatsApp Process Message Error: {str(e)}\n{frappe.get_traceback()}", 
...
#     try:
#         settings = frappe.get_single("WhatsApp Settings")
#         # Try different field names that might contain your business number
#         return (getattr(settings, 'business_number', '') or 
#                 getattr(settings, 'phone_number', '') or 
#                 getattr(settings, 'whatsapp_number', '') or 
#                 "917510758708")  # Your fallback number from the contact we saw
#     except:
#         return "917510758708"  # Fallback number
...
#             # If this is a campaign message, update the campaign recipient status
#             campaign_recipients = frappe.get_all(
#                 "WhatsApp Campaign Recipient",
#                 filters={"status": ["!=", frappe_status]},
#                 fields=["name", "parent"]
#             )
            
#             for recipient in campaign_recipients:
#                 frappe.db.set_value("WhatsApp Campaign Recipient", recipient.name, "status", frappe_status)
                
#                 # Update campaign stats
#                 if frappe_status in ["Delivered", "Read"]:
#                     campaign = frappe.get_doc("WhatsApp Campaign", recipient.parent)
                    
#                     if frappe_status == "Delivered":
#                         campaign.delivered = (campaign.delivered or 0) + 1
#                     elif frappe_status == "Read":
...
#         return True
#     except Exception as e:
#         frappe.log_error(f"WhatsApp Status Update Error: {str(e)}", "WhatsApp Status Update Failed")
#         return False

# def find_or_create_contact(phone_number):
#     """Find or create a Whatsapp Contact"""
#     contacts = frappe.get_all(
#         "Whatsapp Contact",  # Fixed: Changed from "Whatsapp Contact" to "Whatsapp Contact"
#         filters={"phone_number": phone_number},
#         fields=["name"]
#     )
...
#     # Create new contact
#     contact = frappe.get_doc({
#         "doctype": "Whatsapp Contact",  # Fixed: Changed from "Whatsapp Contact" to "Whatsapp Contact"
#         "phone_number": phone_number,
#         "opt_in_status": "Subscribed",  # They messaged us first, so they're opted in
#         "last_message_date": frappe.utils.now_datetime()
#     })
...
# def check_campaign_response(contact_name, content):
#     """Check if this is a response to a campaign message"""
#     # Get the contact's phone number
#     phone_number = frappe.get_value("Whatsapp Contact", contact_name, "phone_number")  # Fixed DocType name
    
#     # Find recent campaign recipients for this contact
...
#             {
#                 "response_received": 1,
#                 "response_text": content[:500]  # Limit to 500 chars
#             }
#         )
        
#         # Update campaign stats
#         campaign = frappe.get_doc("WhatsApp Campaign", recipients[0].parent)
#         campaign.responses = (campaign.responses or 0) + 1
        
#         if campaign.total_sent > 0:
#             campaign.conversion_rate = (campaign.responses / campaign.total_sent) * 100
